...
import {
  getPrimaryDependency,
  PrimaryDependency,
} from './utils/get-primary-dependency';
...
const ImageByPrimaryDependency: Record<PrimaryDependency, string> = {
  [PrimaryDependency.REACT]: reactLogo,
  [PrimaryDependency.NODE]: nodeImage,
  [PrimaryDependency.NEXT]: nextLogo,
};
...
export const getElementDefinitions = (
  ...
  const getIsInternalDependency = (dep: Dependency): boolean => {
    return packages.some((package_) => package_.name === dep.name);
  };
  ...
  try {
    ...
    const nonUniqueEdges: EdgeDefinition[] = packages.flatMap((package_) => {
      const getEdge = (dependency: Dependency) => {
        return {
          data: {
            ...dependency,
            id: `${package_.name}->${dependency.name}`,
            source: package_.name,
    ...
    const edges = uniqBy(
      nonUniqueEdges,
      (item: EdgeDefinition) => item.data.id
    );
    return [...nodes, ...edges];
  } catch (error) {
    console.error(error);
