...
Here's how we recommend getting up an running in a repository that has multiple collaborators.
...
<Tabs items={['npm', 'yarn', 'pnpm']} storageKey="selected-pkg-manager">
  <Tab>
    ```bash copy npm install commonality commonality-checks-recommended
    --save-dev ```
  </Tab>
  <Tab>
    ```bash copy yarn add commonality commonality-checks-recommended --dev
    --ignore-workspace-root-check ```
  </Tab>
  <Tab>
    ```bash copy pnpm add commonality commonality-checks-recommended --save-dev
    --ignore-workspace-root-check ```
  </Tab>
</Tabs>
