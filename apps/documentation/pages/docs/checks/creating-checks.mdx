...
Checks are defined your [project configuration file](/docs/api/configuration) at the root of your project.
You can use [selectors](/docs/selectors) to run checks against all the packages in your project or a subset of packages.

<Callout type="info" emoji="💡">
  Leverage the [utility checks](/docs/commonality-checks-recommended#utility) from our [`commonality-checks-recommended`](/docs/commonality-checks-recommended) package to quickly get up and running for simple use cases.
</Callout>
...
To make a check fixable just add a [`fix`](/docs/api/checks#fix) property to your check. This function will only run against a package if the result of [`validate`](/docs/api/checks#validate) is falsey.
...
We provide utilities like [`json`](/docs/api/checks#json), and [`text`](/docs/api/checks#text) to make it easy to read and write files within checks.
...
As an example, below is the source code for our [`hasJsonFile`](/docs/commonality-checks-recommended#has-json-file) utility check.
