...
A constraint can `allow` and `disallow` packages using [selectors](/docs/selectors), however if a dependency matches a `disallow` selector the constraint will fail even if it is allowed by an `allow` filter.
...
If any direct or transitive dependency matches a [selector](/docs/selectors) within the `disallow` array, then the constraint will fail.
...
![A graph that shows that transitive dependencies are traversed with disallow constraints](/disallow-constraint.png)
