...
`createTestCheck` wraps your checks and decorates these functions with sensible defaults for the `CheckContext` that is passed to your [message](/reference/check-object#message), [validate](/reference/check-object#validate) and [fix](/reference/check-object#fix) functions that you can override.
