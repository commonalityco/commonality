...
Checks are defined your [project configuration file](/sdk/configuration) at the root of your project.
You can use [selectors](/selectors) to run checks against all the packages in your project or a subset of packages.

<Tip>
  Leverage the [utility checks](/commonality-checks-recommended#utility) from our [commonality-checks-recommended](/commonality-checks-recommended) package to quickly get up and running for simple use cases.
</Tip>
...
1. [**name**](/sdk/checks#name): A unique identifier for the check, we use this to parallelize checks and prevent write collisions.

2. [**validate**](/sdk/checks#validate): A function that should return either a truthy or falsey value indicating whether or not the check is valid.

3. [**message**](/sdk/checks#message): A string or function that that provides more context about the check.

<Note>
  You can view all available check properties and more examples in our
  [reference documentation](/sdk/checks).
</Note>
...
To make a check fixable just add a [fix](/sdk/checks#fix) property to your check. This function will only run against a package if the result of [validate](/sdk/checks#validate) is falsey.
...
We provide utilities like [json](/sdk/checks#json), and [text](/sdk/checks#text) to make it easy to read and write files within checks.

## Customizing messages

Messages can be provided as a string for simple checks or constructed dynamically using a function that is passed [CheckContext](/sdk/checks#checkcontext) and returns a Message object.
...
You can return a [Message](/sdk/checks#message-1) object from the [message](/sdk/checks#message) function to dynamically construct the output shown below.
...
Use the [diff](/sdk/checks#diff) utility to suggest file edits needed to pass checks. The `suggestion` property is also useful for showing how a [fix](/sdk/checks#fix) function will modify a file.

The [diff](/sdk/checks#diff) function will return a pretty printed string where unique properties and values in the second argument appear in red and are prefixed with `+`."
...
As an example, below is the source code for our [hasJsonFile](/commonality-checks-recommended#has-json-file) utility check.
